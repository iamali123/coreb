...
import SalesInvoiceItemDetailsList from "./SalesInvoiceItemDetailsList";

function SalesInvoiceItemDetailsForm(props) {
  ...
  const itemDetailFormik = useFormik({
    initialValues: salesInvoiceItemDetailsInitialValues,
    validationSchema: SalesInvoiceItemDetailValidationSchema,
    onSubmit: (values, { resetForm }) => {
      let isDuplicateEntry = props.SalesInvoiceMaterials.filter(
        (material) => material.itemDetailId === values.itemDetailId
      ).length;
      if (isDuplicateEntry > 0) {
        props.setSalesInvoiceMaterials(
          props.SalesInvoiceMaterials.map((material) => {
  ...
  const EditSalesInvoiceItemDetail = (rowData) => {
    ...
    itemDetailFormik.setFieldValue("salesInvoiceId", rowData.salesInvoiceId);
  ...
  return (
    <>
      ...
      <SalesInvoiceItemDetailsList
        SelectedData={props.SelectedData}
        setSelectedData={props.setSelectedData}
        SalesInvoiceMaterials={props.SalesInvoiceMaterials}
        setSalesInvoiceMaterials={props.setSalesInvoiceMaterials}
        EditSalesInvoiceItemDetail={EditSalesInvoiceItemDetail}
      />
    </>
  );
}
...
export default SalesInvoiceItemDetailsForm;
