...
import SalesQuotationItemDetailsList from "./SalesQuotationItemDetailsList";
...
function SalesQuotationItemDetailsForm(props) {
  const classes = useStyles();
  const dispatch = useDispatch();
  const [run, setrun] = useState(false);

  //const [qtyRef, setqtyRef] = UseFocus();
  //const [priceRef, setpriceRef] = UseFocus();
  //let priceRef = UseFocus();
  ...
  useEffect(() => {
    GetBOMMaterials(props.companyId)
      ...
      .catch((error) => {
        console.log("Error Sales Quotation Matrials", error.response);
      });
  }, []);
  ...
  const itemDetailFormik = useFormik({
    initialValues: salesQuotationItemDetailsInitialValues,
    validationSchema: salesQuotationItemDetailValidationSchema,
    onSubmit: (values, { resetForm }) => {
      console.log("values", values);
      let isDuplicateEntry = props.SalesQuotationMaterials.filter(
        (material) => material.itemDetailId === values.itemDetailId
      ).length;
      if (isDuplicateEntry > 0) {
        props.setSalesQuotationMaterials(
          props.SalesQuotationMaterials.map((material) => {
  ...
  const EditSalesQuotationItemDetail = (rowData) => {
  ...
  return (
    <>
        ...
        >
          ...
          <Grid xs={2} sm={2} md={2} lg={2}>
            <TextField
              ...
              onChange={(x) => {
                ...
                itemDetailFormik.setFieldValue("quantity", v);
              }}
              // onChange={itemDetailFormik.handleChange}
              // onBlur={() => {
          ...
          <Grid xs={2} sm={2} md={2} lg={2}>
            <TextField
              ...
              id="unitPrice"
              label="Unit Price"
              value={itemDetailFormik.values.unitPrice}
              type="number"
              onChange={itemDetailFormik.handleChange}
              onKeyDown={(event) => {
                const ENTER_KEY = 13;
      ...
      <SalesQuotationItemDetailsList
        SelectedData={props.SelectedData}
        setSelectedData={props.setSelectedData}
        SalesQuotationMaterials={props.SalesQuotationMaterials}
        setSalesQuotationMaterials={props.setSalesQuotationMaterials}
        EditSalesQuotationItemDetail={EditSalesQuotationItemDetail}
      />
    </>
  );
}
...
export default SalesQuotationItemDetailsForm;
