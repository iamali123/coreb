...
import SalesOrderItemDetailsList from "./SalesOrderItemDetailsList";

function SalesOrderItemDetailsForm(props) {
  ...
  const itemDetailFormik = useFormik({
    initialValues: salesOrderItemDetailsInitialValues,
    validationSchema: SalesOrderItemDetailValidationSchema,
    onSubmit: (values, { resetForm }) => {
      let isDuplicateEntry = props.SalesOrderMaterials.filter(
        (material) => material.itemDetailId === values.itemDetailId
      ).length;
      if (isDuplicateEntry > 0) {
        props.setSalesOrderMaterials(
          props.SalesOrderMaterials.map((material) => {
        ...
        props.setSalesOrderMaterials([...props.SalesOrderMaterials, values]);
        resetForm();
        setrun(!run);
      }
    },
  ...
  const EditSalesOrderItemDetail = (rowData) => {
    ...
    itemDetailFormik.setFieldValue("salesOrderId", rowData.salesOrderId);
  ...
  return (
    <>
      ...
      <SalesOrderItemDetailsList
        SelectedData={props.SelectedData}
        setSelectedData={props.setSelectedData}
        SalesOrderMaterials={props.SalesOrderMaterials}
        setSalesOrderMaterials={props.setSalesOrderMaterials}
        EditSalesOrderItemDetail={EditSalesOrderItemDetail}
      />
    </>
  );
}
...
export default SalesOrderItemDetailsForm;
