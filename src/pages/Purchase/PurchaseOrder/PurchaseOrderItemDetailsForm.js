...
import {
  purchaseOrderItemDetailsInitialValues,
  PurchaseOrderItemDetailValidationSchema,
...
import PurchaseOrderItemDetailsList from "./PurchaseOrderItemDetailsList";
function PurchaseOrderItemDetailsForm(props) {
  ...
  const itemDetailFormik = useFormik({
    ...
    validationSchema: PurchaseOrderItemDetailValidationSchema,
    enableReinitialize: true,
    onSubmit: (values, { resetForm }) => {
      let isDuplicateEntry = props.PurchaseOrderMaterials.filter(
        (material) => material.itemDetailId === values.itemDetailId
      ).length;
      if (isDuplicateEntry > 0) {
        props.setPurchaseOrderMaterials(
          props.PurchaseOrderMaterials.map((material) => {
  ...
  const EditPurchaseOrderItemDetail = (rowData) => {
  ...
  return (
    <>
      ...
      <PurchaseOrderItemDetailsList
        SelectedData={props.SelectedData}
        setSelectedData={props.setSelectedData}
        PurchaseOrderMaterials={props.PurchaseOrderMaterials}
        setPurchaseOrderMaterials={props.setPurchaseOrderMaterials}
        EditPurchaseOrderItemDetail={EditPurchaseOrderItemDetail}
      />
    </>
  );
}
...
export default PurchaseOrderItemDetailsForm;
